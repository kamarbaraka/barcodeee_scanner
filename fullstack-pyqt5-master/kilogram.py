...
class kilogram(QWidget):
    ...
    def output(self):
        try: 
            barcode = int(self.input.text())
            kgs = int(self.input2.text())
            data1 =databaseApi2.DatabaseApi("./database/database", "./databaseApi/barcode_images")
            data1.set_kg(barcode,kgs)
            QMessageBox.information(None, "saved", "kilograms saved successfully")
        except Exception as e:
            QMessageBox.warning(None, "warning", "Your barcode is either wrong or empty. Please try again")
